...
class SimplexSolver:
    """
    Implementa el Método Simplex en Dos Fases.

    Fase 1: agrega variables artificiales y minimiza su suma para encontrar
            una Solución Básica Factible (SBF).
    Fase 2: sobre esa SBF, optimiza la función objetivo original.
    """
    ...
    def solve(self) -> Result:
        ...
        # Convertir a maximización: Min Z = cᵀx  ⟺  Max W = −cᵀx
        if self.model.sense == "min":
            c_obj = [-ci for ci in self.model.vector_c]
        else:
            c_obj = list(self.model.vector_c)

        tableau, basis, n_slack, n_art, art_indices = self._build_tableau(n, m)

        # Fase 1 (solo si hay restricciones >= o =)
        if n_art > 0:
            if not self._phase1(tableau, basis, n, n_slack, art_indices, m):
                self.result.status = "Infeasible"
                return self.result

        # Fase 2
        self._phase2(tableau, basis, n, n_slack, n_art, m, c_obj)
        return self.result
    ...
    def _build_tableau(self, n, m):
        ...
        return tableau, basis, n_slack, n_art, art_indices

    # ── Fase 1 ─────────────────────────────────────────────────────────────

    def _phase1(self, tableau, basis, n, n_slack, art_indices, m):
        """
        Minimiza Σ artificiales.
        Si el mínimo resultante es 0 → SBF encontrada (factible).
        Si > 0 → problema infactible.
        """
        cols = len(tableau[0])
        ...
        # Eliminar básicas de la fila objetivo
        ...
        tableau.append(obj)
        self._simplex_iterations(tableau, basis, m, allowed_cols=None)

        z_phase1 = -tableau[-1][-1]
        tableau.pop()

        if z_phase1 > Fraction(0):
            return False

        # Sacar artificiales degeneradas de la base si las hay
        for i, bv in enumerate(basis):
            if bv in art_indices:
                for j in range(n + n_slack):
                    if tableau[i][j] != 0:
                        self._pivot(tableau, basis, i, j, m)
                        break
    ...
    def _phase2(self, tableau, basis, n, n_slack, n_art, m, c_obj):
        ...
        for i, bv in enumerate(basis):
            ...
            if coef != 0:
                for k in range(cols):
                    obj[k] -= coef * tableau[i][k]

        tableau.append(obj)

        # Solo variables de decisión y holgura/exceso (sin artificiales)
        allowed = list(range(n + n_slack))
        self._simplex_iterations(tableau, basis, m, allowed_cols=allowed)

        self.result.status = self.result.status or "Optimal"
...
def solve(model: LPModel) -> Result:
    """
    Resuelve un LPModel y devuelve un Result.
